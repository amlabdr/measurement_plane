...
class TaskSchedule:
    def __init__(self, schedule:str) -> None:
        self.start, self.stop, self.periodicity = None, None, None
    ...
    def parse_schedule(self, schedule: str):
        ...
        if len(parts) == 3 and parts[2].strip():
            periodicity_match = re.match(r'(\d+)([smhd])', parts[2].strip())
            if periodicity_match:
                value, unit = periodicity_match.groups()
                value = int(value)
                if unit == 's':
        ...
        return self.start, self.stop, self.periodicity
